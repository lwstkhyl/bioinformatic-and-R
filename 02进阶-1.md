...
- [基础绘图](#基础绘图)
    - [画图函数](#画图函数)
      ...
      - [hist](#hist)
      - [plot（重点）](#plot重点)
    - [一些关于画图的基础知识](#一些关于画图的基础知识)
      - [高低图层](#高低图层)
      - [使用系统函数设定图形相关参数](#使用系统函数设定图形相关参数)
      - [图形设备](#图形设备)
...
本节中使用的例子：`aapl`表
![dplyr包例](./md-image/dplyr包例.webp "dplyr包例"){:width=250 height=250}
##### arrange
...
aapl %>% arrange(-Volume)
```
![arrange例](./md-image/arrange例.webp "arrange例"){:width=250 height=250}
##### select
...
```
![select例](./md-image/select例.webp "select例"){:width=250 height=250}
...
```
![filter例](./md-image/filter例.webp "filter例"){:width=250 height=250}
``` r
aapl %>% filter((Close>=150) & (Close>Open))  # 选择close列>=150且close列值>open列值的行
```
![filter例2](./md-image/filter例2.webp "filter例2"){:width=250 height=250}
##### mutate
...
```
![mutate例](./md-image/mutate例.webp "mutate例"){:width=250 height=250}
``` r
aapl %>% mutate(n=row_number())  # 为每行数据添加行数标记
```
![mutate例1](./md-image/mutate例1.webp "mutate例1"){:width=250 height=250}
其它的mutate函数：
...
例：weather数据集
![summarise与group_by](./md-image/summarise与group_by.webp "summarise与group_by"){:width=250 height=250}
``` r
# 按city分组，计算温度的平均值，存入新列mean_temperature中
weather %>% group_by(city) %>% summarise(mean_temperature = mean(temperature))
```
![summarise与group_by例](./md-image/summarise与group_by例.webp "summarise与group_by例"){:width=125 height=125}
当不指定分组，直接summarise时，按所有数据为一组，即计算所有数据的平均值等
``` r
weather %>% summarise(mean_temperature = mean(temperature))
```
![summarise与group_by例2](./md-image/summarise与group_by例2.webp "summarise与group_by例2"){:width=100 height=100}
...
例：
宽数据为
![宽数据](./md-image/宽数据.webp "宽数据"){:width=80 height=250}
长数据为
![长数据](./md-image/长数据.webp "长数据"){:width=80 height=200}
宽变长：
``` r
long <- wide %>% 
...
原始数据：
![原始数据](./md-image/原始数据.webp "原始数据"){:width=80 height=200}
``` r
data %>%
  separate(Quarter, c("Time_Interval", "Interval_ID"));
```
分隔后数据：
![分隔后数据](./md-image/分隔后数据.webp "分隔后数据"){:width=80 height=200}
...
```
![apply函数结果2](./md-image/apply函数结果2.png "apply函数结果2"){:width=100 height=100}
...
```
![barplot](./md-image/barplot.png "barplot"){:width=300 height=300}
例2：
准备数据：
``` r
...
boxplot(data)
```
![boxplot](./md-image/boxplot.png "boxplot"){:width=200 height=200}
例2：复杂的箱线图 
准备数据：
``` r
...
```
![boxplot2](./md-image/boxplot2.png "boxplot2"){:width=150 height=150}
``` r
# 绘制箱线图
boxplot(
...
```
![boxplot3](./md-image/boxplot3.png "boxplot3"){:width=300 height=300}
###### pie
...
pie(data);
```
![boxplot4](./md-image/boxplot4.png "boxplot4"){:width=200 height=200}
###### hist
创建直方图
``` r
hist(x, # 向量或因子
...
```
![boxplot5](./md-image/boxplot5.png "boxplot5"){:width=300 height=300}
###### plot（重点）
...
```
![plot参数](./md-image/plot参数.png "plot参数"){:width=350 height=350}
还可以：
``` r
with(
  swiss, 
