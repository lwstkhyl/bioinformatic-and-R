...
# is_tumor是由bool值组成的vector，第n个元素说明counts中第n列和surv_data中第n行是不是tumor数据
...
tumor_counts$gene_id <- str_split(tumor_counts$gene_id, pattern = '[.]', simplify = TRUE)[, 1];  # 将基因id中`.`后的部分去掉，使其格式同gene_name
...
现在观察到tumor_counts和gene_name都有gene_id这列，可以将现在观察到tumor_counts和gene_name都有gene_id这列与gene_name表进行连接(join函数)
```
tumor_counts <- left_join(tumor_counts, gene_name, by = "gene_id");  # 也可以用right_join等
```
tumor_counts：

为方便查看，现将基因名称列调到gene_id列后
```
tumor_counts <- relocate(tumor_counts, gene_name, .after = gene_id);
```
tumor_counts：
...
可以看到tumor_counts_agg比tumor_counts少了很多行，原行名基因id就被删除了
最后将基因名转成行名
```
