...
最终结果：
![临床数据2](./md-image/临床数据2.png){:width=250 height=250}
**数据处理**：
...
- 选中`stage_event`列，点击`数据`中的`分列`，选中`分隔符号`，`下一步`
  ![临床数据3](./md-image/临床数据3.png){:width=250 height=250}

- 选定分隔符号为`其他`，输入`T`，点击`完成`，即可按T进行分隔
  ![临床数据4](./md-image/临床数据4.png){:width=250 height=250}
  ![临床数据7](./md-image/临床数据7.png){:width=250 height=250}
- 再用相同方法，选中刚刚分隔的列，依次按`N`和`M`分隔
  ![临床数据8](./md-image/临床数据8.png){:width=250 height=250}
- 更改它们的列名分别为T/N/M
  ![临床数据9](./md-image/临床数据9.png){:width=250 height=250}
...
```
每个df：
![miRNA数据1](./md-image/miRNA数据1.png){:width=250 height=250}
更改后的每个df：
![miRNA数据2](./md-image/miRNA数据2.png){:width=250 height=250}
想实现的效果：
- `miRNA_region`作行名
...
```
![miRNA数据3](./md-image/miRNA数据3.png){:width=250 height=250}
其它处理：
- 删去最后三行无用数据
- 删去行名中的`mature,`
...
```
![miRNA数据4](./md-image/miRNA数据4.png){:width=250 height=250}
...
mh：
![miRNA数据5](./md-image/miRNA数据5.png){:width=250 height=250}
data：
![miRNA数据6](./md-image/miRNA数据6.png){:width=250 height=250}
行名中`-3p`/`-5p`是3'/5'端的意思
...
```
![miRNA数据7](./md-image/miRNA数据7.png){:width=250 height=250}
...
```
![miRNA数据8](./md-image/miRNA数据8.png){:width=250 height=250}
**保存数据**：
- 将行名作为一列
- 将列名中的`.`替换为`-`
...
```
###### GEO数据
需要包：`tidyverse`、`GEOquery`、`hgu133plus2.db`
```
