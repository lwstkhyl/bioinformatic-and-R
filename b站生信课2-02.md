...
![单因素cox回归1](./md-image/单因素cox回归1.png){:width=250 height=250}
![单因素cox回归2](./md-image/单因素cox回归2.png){:width=250 height=250}
...
rt <- cbind(cli, tpm);  # 合并
```
![单因素cox回归3](./md-image/单因素cox回归3.png){:width=250 height=250}
**cox回归**：
```{r}
p.value <- 0.01;  # p值的阈值
outTab <- data.frame();  # 结果矩阵
...
```
![单因素cox回归4](./md-image/单因素cox回归4.png){:width=250 height=250}
**画图的数据准备**：基因名、HR、p值
```{r}
...
dev.off();
```
![单因素cox回归5](./md-image/单因素cox回归5.png){:width=250 height=250}
