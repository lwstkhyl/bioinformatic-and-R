...
在竖线左边则HR<1，右边则>1
**准备的数据**：
![单因素Cox回归分析2](./md-image/单因素Cox回归分析2.png){:width=400 height=400}
...
```
![expr_norm](./md-image/expr_norm.png){:width=300 height=300}
**获取差异化表达的、肿瘤组的基因表达数据**：
``` r
...
```
![surv_for_cox](./md-image/surv_for_cox.png){:width=300 height=300}
**最后只需将两组数据列合并即可**：
``` r
...
pdf(file = "C:\\Users\\WangTianHao\\Documents\\GitHub\\R-for-bioinformatics\\b站生信课01\\data\\cv_fit.pdf", width = 10);
plot(cv_fit);
dev.off();
# 回归系数路径图
fit <- glmnet(x = t(dat_cox_sur), y = dat_cox_2$os);
pdf(file = "C:\\Users\\WangTianHao\\Documents\\GitHub\\R-for-bioinformatics\\b站生信课01\\data\\fit.pdf", width = 10);
plot(fit, xvar = 'lambda');
dev.off();
# lasso回归
