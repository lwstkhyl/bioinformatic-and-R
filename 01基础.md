...
- [基础语段](#基础语段)
    ...
    - [if](#if)
    - [函数](#函数)
- [data.frame和tibble基础](#dataframe和tibble基础)
    - [创建](#创建-2)
    - [添加](#添加)
    - [取/替换行列](#取替换行列)
    - [相互转换](#相互转换)
...
- [factor](#factor)
    - [创建](#创建-3)
    - [新增的值必须是levels中的已有值](#新增的值必须是levels中的已有值)
    - [levels的顺序决定了排序的顺序](#levels的顺序决定了排序的顺序)
    - [forcats包](#forcats包)
      - [一些有用的函数](#一些有用的函数)
      - [修改因子向量顺序](#修改因子向量顺序)
      - [修改因子向量名称](#修改因子向量名称)
...
```
![tb创建2](./md-image/tb创建2.png){:width=100 height=100}
...
```
![df添加](./md-image/df添加.png){:width=150 height=150}
``` r
m <- cbind(1, 1:7) ;
m <- cbind(m, 8:14) ;
```
![df添加2](./md-image/df添加2.png){:width=150 height=150}
...
```
![tb添加](./md-image/tb添加.png){:width=100 height=100}
##### 取/替换行列
df与tb的取法基本相同
可以通过整数索引取，也可通过行/列名取
...
```
替换前：
![fct_collapse1](./md-image/fct_collapse1.png "fct_collapse1"){:width=173 height=173}
替换后：
