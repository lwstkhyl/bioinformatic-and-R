...
![GTEx数据下载和整理1](./md-image/GTEx数据下载和整理1.png){:width=220 height=220}
...
![GTEx数据下载和整理2](./md-image/GTEx数据下载和整理2.png){:width=220 height=220}
![GTEx数据下载和整理3](./md-image/GTEx数据下载和整理3.png){:width=220 height=220}
![GTEx数据下载和整理4](./md-image/GTEx数据下载和整理4.png){:width=220 height=220}
```{r}
...
```
![GTEx数据下载和整理6](./md-image/GTEx数据下载和整理6.png){:width=200 height=200}
...
```
![GTEx数据下载和整理7](./md-image/GTEx数据下载和整理7.png){:width=200 height=200}
...
![泛癌差异基因箱线图2](./md-image/泛癌差异基因箱线图2.png){:width=200 height=200}
![泛癌差异基因箱线图3](./md-image/泛癌差异基因箱线图3.png){:width=200 height=200}
...
dev.off();
```
![WGCNA4](./md-image/WGCNA4.png){:width=200 height=200}
红框中的样本聚类得比较好，蓝框为离群值
![WGCNA5](./md-image/WGCNA5.png){:width=200 height=200}
...
```
![WGCNA6](./md-image/WGCNA6.png){:width=200 height=200}
...
dev.off();
```
![WGCNA8](./md-image/WGCNA8.png){:width=200 height=200}
...
```
![WGCNA9](./md-image/WGCNA9.png){:width=200 height=200}
可以看到应选取的power值为6
基因聚类：
```{r}
# TOM矩阵
...
dev.off();
```
![WGCNA10](./md-image/WGCNA10.png){:width=200 height=200}
动态剪切模块识别：
```{r}
#模块基因数目
...
dev.off();
```
![WGCNA11](./md-image/WGCNA11.png){:width=200 height=200}
...
dev.off();
```
![WGCNA12](./md-image/WGCNA12.png){:width=200 height=200}
...
MEs <- mergedMEs;
```
![WGCNA13](./md-image/WGCNA13.png){:width=200 height=200}
`MEs`：
![WGCNA14](./md-image/WGCNA14.png){:width=200 height=200}
提取共有样本：
```{r}
# 重新读入临床数据
...
dev.off();
```
![WGCNA15](./md-image/WGCNA15.png){:width=200 height=200}
...
```
`geneInfo`：
![WGCNA16](./md-image/WGCNA16.png){:width=200 height=200}
`View(as.data.frame(modGenes))`：（共有三个，这里以其中为1个为例）
![WGCNA17](./md-image/WGCNA17.png){:width=200 height=200}
计算GS和MM：
...
以MEturquoise（青色的那个组），性别为例（因为在上面的相关图中，只有这个块的p值<0.05）
```{r}
module <- "turquoise";
...
```
`Sex_turquoise.pdf`：
![WGCNA18](./md-image/WGCNA18.png){:width=200 height=200}
...
```
![WGCNA19](./md-image/WGCNA19.png){:width=200 height=200}
得到在青色模块中与性别相关的关键基因
