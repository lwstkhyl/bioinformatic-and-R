...
`lncRNA`：
![mRNAlncRNA共表达分析2](./md-image/mRNAlncRNA共表达分析2.png){:width=200 height=200}
进行分析：
```{r}
# 筛选阈值
corFilter <- 0.3;
...
```
`outTab`：
![mRNAlncRNA共表达分析3](./md-image/mRNAlncRNA共表达分析3.png){:width=200 height=200}
...
dev.off();
```
![mRNAlncRNA共表达分析5](./md-image/mRNAlncRNA共表达分析5.png){:width=200 height=200}
上面是mRNA，下面是lncRNA，右边图例是mRNA的，因为lncRNA数量太多，没有显示它们的名称
![mRNAlncRNA共表达分析6](./md-image/mRNAlncRNA共表达分析6.png){:width=200 height=200}
